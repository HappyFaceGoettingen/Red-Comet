...
class GridLCGCopyHandler(GridSubprocessBaseHandler):
    ...
    def showFiles (self, dstPath):
        ...
        self.commandArgs = "lcg-ls " + self.__protocol + "://" + self.__dstHost + ":" + self.__dstPort + self.__dstPath
    ...
    def copyFromLocalToRemote(self, srcHost, localPath, srcPath):     
        self.commandArgs = 'lcg-cp --verbose -n 1 file:///' + localPath + '  ' + self.__protocol + "://" + self.__srcHost + ":" + self.__dstPort + srcPath 
        self.logger.debug("Executed command = " +  str(self.commandArgs))
        self.execute()
        try: 
           (data,error) = self.gridProcess.communicate()
    ...
    def copyFile(self, srcPath, dstPath):
        ...
        self.__dstPath = dstPath              
        self.commandArgs = 'lcg-cp --verbose -n 1 ' + self.__protocol + "://" + self.__srcHost + ":" + self.__srcPort + self.__srcPath + "  " +  self.__protocol + "://" + self.__dstHost + ":" + self.__dstPort + self.__dstPath 
    ...
    def copyFileAndCheckExistance(self, srcPath, fileName, dstPath):
        self.__srcPath = srcPath
        self.__dstPath = dstPath
        data, error = self.copyFile(srcPath+fileName, dstPath+ "lcg_" +fileName)
        if not error:
...
def main():
    print "GridLCGCopyHandler"
    logging.basicConfig(level=logging.DEBUG)
    logging.root.setLevel(logging.DEBUG)
   
    Object = GridLCGCopyHandler() 
    Object.setHostsAndPorts("se-goegrid.gwdg.de", "8443", "se-goegrid.gwdg.de", "8443", "srm")
    #Object.copyFromLocalToRemote("/home/haykuhi/Desktop/6.jpg", "/pnfs/gwdg.de/data/atlas/atlasscratchdisk/test_haykuhi/8.jpg")
